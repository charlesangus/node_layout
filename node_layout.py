...
def _reorder_inputs_mask_last(input_slot_pairs, node, all_side, fan_active=False):
    ...
    so the layout engine can position them to the left of the consumer.  Relative
...
def layout_upstream(scheme_multiplier=None):
    """Run bbox layout from the selected node up through the upstream graph."""
    import node_layout_engine
    node_layout_engine.BboxEngine().layout_upstream(scheme_multiplier=scheme_multiplier)
...
def layout_selected(scheme_multiplier=None):
    """Run bbox layout on each selection root independently within the selection scope."""
    import node_layout_engine
    node_layout_engine.BboxEngine().layout_selected(scheme_multiplier=scheme_multiplier)
...
def layout_selected_horizontal(scheme_multiplier=None):
    """Lay out the selected nodes as a horizontal spine; non-spine inputs stack vertically above.

    Writes mode='horizontal' to the spine nodes so subsequent layout_selected() replays it.
    """
    import node_layout_engine
    node_layout_engine.BboxEngine().layout_selected_horizontal(scheme_multiplier=scheme_multiplier)


def layout_selected_horizontal_place_only(scheme_multiplier=None):
    """Same as layout_selected_horizontal but moves attached subtrees as rigid units."""
    import node_layout_engine
    node_layout_engine.BboxEngine().layout_selected_horizontal_place_only(
