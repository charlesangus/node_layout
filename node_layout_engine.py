"""Leaf-first bbox composition layout engine.

Runs a post-order DFS that builds ``Subtree`` objects (bounding box +
nodes-dict) and merges them into the parent frame.

Helpers reused from ``node_layout``:
...
def _filtered_input_pairs(node, ctx: LayoutContext):
    """Return (slot, input_node) pairs after node_filter, mask-reorder, and fan handling."""
...
# ---------------------------------------------------------------------------
# Engine — top-level layout entry points.
# ---------------------------------------------------------------------------
...
class BboxEngine:
    ...
    def _run_upstream(self, root, scheme_multiplier, current_group):
        ...
        # No output Dot is inserted below a horizontal seed: the seed sits
        # naturally above its consumer in this layout, so a Dot at consumer
        # mid-Y would overlap the consumer tile. The leftmost-spine boundary
        # Dot inserted by ``layout_horizontal`` provides the only routing
        # turn the chain needs.

        # The consumer's mixed vertical+horizontal inputs are handled natively
        # by the recursion; nothing extra to do here. A vertical consumer with
