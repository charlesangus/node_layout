...
def _subtree_margin_count() -> int:
    """Node-count argument that makes _subtree_margin match Claude behavior."""
    return node_layout_prefs.prefs_singleton.get("scaling_reference_count")


def _scaled_subtree_margin(node, slot: int, ctx: LayoutContext) -> int:
    return int(
        node_layout._subtree_margin(
            node,
            slot,
            _subtree_margin_count(),
            mode_multiplier=ctx.scheme_for(node),
        ) * ctx.v_scale_for(node)
    )
...
def _record_side_dot_gap(dot, consumer, slot: int, ctx: LayoutContext):
    gap = _scaled_subtree_margin(consumer, slot, ctx)
...
def _is_prepared_side_dot(node) -> bool:
    return (
        node is not None
        and node.Class() == "Dot"
        and node.knob(_SIDE_DOT_KNOB_NAME) is not None
        and node.input(0) is not None
    )


def _layout_for_place_only(root, ctx: LayoutContext) -> Subtree:
    if _is_layout_routing_dot(root):
        return layout(root, ctx)
    return _snapshot_existing_subtree(root, ctx)
...
def layout_vertical(node, ctx: LayoutContext) -> Subtree:
    ...
    prepared_side_dot_inputs = [_is_prepared_side_dot(inp) for inp in inputs]
    layout_roots = [
        inp.input(0) if prepared_side_dot_inputs[i] else inp
        for i, inp in enumerate(inputs)
    ]
    n = len(inputs)
    child_subtrees = [layout(inp, ctx) for inp in layout_roots]
    ...
    side_margins_v = [
        _scaled_subtree_margin(node, slot, ctx)
        for slot in actual_slots
    ]
    ...
    if is_fan:
        ...
        raw_gap_b = node_layout.vertical_gap_between(
            layout_roots[non_mask_start], node, snap, scheme
        )
    ...
    if all_side:
        ...
        x_positions = [
            node_layout._center_x(layout_roots[0].screenWidth(), 0, node_w)
        ]
    elif n == 2:
        root_x_0 = node_layout._center_x(layout_roots[0].screenWidth(), 0, node_w)
        ...
        root_x_b = node_layout._center_x(
            layout_roots[non_mask_start].screenWidth(), 0, node_w
        )
    ...
    for i, child in enumerate(child_subtrees):
        ...
        if prepared_side_dot_inputs[i]:
            ...
            dot_center_x = (
                translated.anchor_out[0] + layout_roots[i].screenWidth() // 2
            )
            dot_x = dot_center_x - dot_w // 2
            dot_y = -(snap - 1) - dot_h if is_fan else (node_h - dot_h) // 2
            nodes_dict[dot] = (dot_x, dot_y)
            ...
            anchor_in_per_slot[actual_slots[i]] = (dot_x, dot_y)
...
def layout_horizontal(root, ctx: LayoutContext) -> Subtree:
    ...
    for spine_node in spine_nodes:
        ...
        for slot in range(1, spine_node.inputs()):
            ...
            if inp is None or not ctx.passes_filter(inp):
                continue
            child_subtree = (
                _layout_for_place_only(inp, side_ctx)
    ...
    # mode) and placed so its bbox clears both the spine node and the full
    # leftmost segment footprint. The routing Dot stays on the spine row while
    # the input subtree sits above it.
    if spine_nodes:
        ...
        if zero_eligible:
            if zero.knob(node_layout._LEFTMOST_DOT_KNOB_NAME) is not None:
                _SIDE_DOT_VERTICAL_GAPS.setdefault(
                    id(zero),
                    max(
                        ctx.snap_threshold - 1,
                        _scaled_subtree_margin(zero, 0, side_ctx),
                    ),
                )
            zero_subtree = (
                _layout_for_place_only(zero, side_ctx)
                if ctx.side_layout_mode == "place_only"
                else layout(zero, side_ctx)
            )
            # Target: zero subtree's bbox right edge clears both the spine tile
            # and the full leftmost segment bbox, including side-input overhangs.
            ...
            leftmost_segment_left = leftmost_x + occupied_intervals[-1][0]
            target_zero_bbox_right = min(
                leftmost_x - step_x,
                leftmost_segment_left - h_gap,
            )
...
def _collect_post_layout_footprint(roots, freeze_blocks) -> dict[int, object]:
    """Collect push-room membership after topology mutation and layout."""
    nodes_by_id: dict[int, object] = {}
    for root in roots:
        for node in node_layout.collect_subtree_nodes(root):
            nodes_by_id[id(node)] = node
    for block in freeze_blocks:
        for member in block.members:
            nodes_by_id.setdefault(id(member), member)
    return nodes_by_id
...
class BboxEngine(node_layout_engine.LayoutEngine):
    ...
    def _run_upstream(self, root, scheme_multiplier, current_group):
        ...
        footprint = _collect_post_layout_footprint([root], freeze_blocks)
        final_node_ids = set(footprint)
        bbox_after_nodes = list(footprint.values())
    ...
    def _run_selected(self, selected, scheme_multiplier, current_group):
        ...
        footprint = _collect_post_layout_footprint(roots, freeze_blocks)
        bbox_after = node_layout.compute_node_bounding_box(list(footprint.values()))
        if bbox_before is not None and bbox_after is not None:
            node_layout.push_nodes_to_make_room(
                set(footprint), bbox_before, bbox_after,
                current_group=current_group,
                freeze_blocks=freeze_blocks,
            )
    ...
    def _run_selected_horizontal(self, scheme_multiplier, side_layout_mode, undo_label):
        ...
        try:
            with current_group:
                ...
                footprint = _collect_post_layout_footprint([root], freeze_blocks)
                bbox_after = node_layout.compute_node_bounding_box(
                    list(footprint.values())
                )
                if bbox_before is not None and bbox_after is not None:
                    node_layout.push_nodes_to_make_room(
                        set(footprint), bbox_before, bbox_after,
