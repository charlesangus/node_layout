...
- [x] **LAYOUT-01**: When a node has 2+ non-mask inputs, all immediate input nodes are placed at the same Y position (fan alignment), with their subtrees extending upward
- [x] **LAYOUT-02**: When a node has 2+ non-mask inputs, the mask input is placed to the left of all non-mask inputs
